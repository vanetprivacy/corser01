...
{
    // int ArgumentsCount=sizeof(argv)/sizeof(*argv); // Ontik changed
    // int ArgumentsCount = 4; // Ontik changed
    if (argc != 4)
    {
        cout<<"ERROR: Please supply four arguments. (Had "<<argc<<")."<<endl;
        return 1;
    }
    // initialize command line variables
    ...
    int counter = 0;
    int anoncount = 0;               // count of all anonymized vehicles // N
    ...
    int silent = 30; // rev:2015-03-21  // S
    ...
    // For SMP-R: Total Overhead = Announcement Overhead
    ...
    // For SMP-R; Total Overhead = Announcement Overhead
    TotalOverheadCounter = AnnouncementOverhead;
    // write output file
