// filename    : gen_stats6.cpp
//               same as gen_stats.cpp except continuous times
//               AND contiguous locations (DSDZ: Do-si-do Zone)
//
//               In DSDZ, group leader (GL) is a "pirate" and 
//               group follower (GF) is a "parrot" 
//               * If GL encounters GF then parrot mimics pirate
//               * If 2 GL's then pirates mimic each other
//               * If 2 GF's then nothing
//               # zones == # pirates; parrots join in groups
//
// author      : george corser, rev:2015-03-07
...
int main(int argc, char *argv[])
{
	...
	int input_type = 2;           // 0=rural, 1=urban, 2=city, 3=test
    bool debug = false;
	
    // declare and open files
    ifstream inData;		         // declare input file object
    ofstream outData;		         // declare output file object
    if (input_type==0)               // rev:2015-02-28
    {
        inData.open("rural.srtt"); 	     // open infile
        outData.open("rural.stat");       // open outfile
    }
    if (input_type==1)
    {
        inData.open("urban.srtt"); 	     // open infile
        outData.open("urban.stat");       // open outfile
    }
    if (input_type==2)
    {
        inData.open("city.srtt"); 	     // open infile
        outData.open("city.stat");       // open outfile
    }
    if (input_type==3)
    {
        inData.open("test.srtt"); 	     // open infile
        outData.open("test.stat");       // open outfile
    }
    ...
    cout << "dsd6 density1 " << radius << " " << zone_qty << setprecision(5) 
        << fixed << " " << avg_k << " " << avg_d << " " << avg_t 
        << " " << anoncount << " " << maxv << endl;
    if (input_type == 1) // urban
        cout << "dsd6 density2 " << radius << " " << zone_qty << setprecision(5) 
        << fixed << " " << avg_k << " " << avg_d << " " << avg_t 
        << " " << anoncount << " " << maxv << endl;
    if (input_type == 2) // city
        cout << "dsd6 density3 " << radius << " " << zone_qty << setprecision(5) 
        << fixed << " " << avg_k << " " << avg_d << " " << avg_t 
        << " " << anoncount << " " << maxv << endl;
    if (input_type == 3) // test
        cout << "dsd6 test " << radius << " " << zone_qty << setprecision(5) 
        << fixed << " " << avg_k << " " << avg_d << " " << avg_t 
        << " " << anoncount << " " << maxv << endl;
